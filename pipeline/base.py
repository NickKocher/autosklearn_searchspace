...
import pipeline.create_searchspace_util as create_searchspace_util
...
class BasePipeline(Pipeline):
    ...
    def __init__(
        ...
        if config is None:
            ...
            # removed check to work with newer version
    ...
    def fit_transformer(self, X, y, fit_params=None):
        ...
        fit_params_steps = self._check_method_params("fit", fit_params)
        Xt = self._fit(X, y, fit_params_steps)
        return Xt, fit_params_steps[self.steps[-1][0]].fit
    ...
    def set_hyperparameters(
        ...
        for node_idx, n_ in enumerate(self.steps):
            ...
            if (
                "init_params"
                in node.get_hyperparameter_search_space.__code__.co_varnames
            ):
    ...
    def _get_base_search_space(
        ...
        matches = create_searchspace_util.get_match_array(
            pipeline=pipeline,
        ...
        for node_idx, n_ in enumerate(pipeline):
            ...
            #  choices are actually legal choices
                ...
                choices_list = create_searchspace_util.find_active_choices(
                    matches,
                    node,
                    node_idx,
                    dataset_properties,
        ...
        if np.sum(matches) < np.size(matches):
            cs = create_searchspace_util.add_forbidden(
                conf_space=cs,
                pipeline=pipeline,
                matches=matches,
                dataset_properties=dataset_properties,
    ...
    def _check_init_params_honored(self, init_params):
        ...
        for key, value in init_params.items():
            ...
            if variable_name not in node_dict or node_dict[variable_name] != value:
                raise ValueError(
                    "Cannot properly set the pair {}->{} via init_params" "".format(
                        key, value
                    )
                )
