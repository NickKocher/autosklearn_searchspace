...
class FeatTypeSplit(AutoSklearnPreprocessingAlgorithm):
    ...
    def __init__(
        ...
        run_foreign_configs = True
        # WIP dirty workaround to allow running of foreign configs, i.e from pipelines that need feature transformers this dataset doesnt
        if run_foreign_configs:
            if "categorical" not in self.feat_type.values():
                self._transformers.append(("categorical_transformer", self.categ_ppl))
            if "numerical" not in self.feat_type.values():
                self._transformers.append(("numerical_transformer", self.numer_ppl))
        if self.config:
